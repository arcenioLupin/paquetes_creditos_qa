create or replace PACKAGE BODY       pkg_sweb_five_mant_pedido AS
  /******************************************************************************
  ...
  ******************************************************************************/
  ...
  BEGIN
    OPEN p_ret_cursor FOR
      SELECT z.num_ficha_vta_veh,
             ...
             --/*Inicio Req. 92036 ebarboza 23 05 2021*/
            (SELECT PKG_GEN_SELECT.FUNC_LVAL_AREA_FAMILIA_MARCA(AX.COD_CIA,'EXCLUFCL',AX.COD_BAUMUSTER,AX.COD_FAMILIA_VEH,AX.COD_AREA_VTA)
            FROM VVE_PEDIDO_VEH AX
            INNER JOIN VVE_FICHA_VTA_PEDIDO_VEH BX
            ON BX.NUM_PEDIDO_VEH = AX.NUM_PEDIDO_VEH
            AND BX.COD_CIA = AX.COD_CIA
            AND BX.COD_PROV = AX.COD_PROV
            AND BX.IND_INACTIVO = 'N'
            WHERE AX.NUM_PEDIDO_VEH = a.num_pedido_veh)fcc_modelo_excluido_sid_tree,
            --/*Fin Req. 92036 ebarboza 23 05 2021*/
             '1' ind_fech_aprob
  ...
  v_asignado_preasig_lineup NUMBER;--<89233>
   -----------------Validar Reserva Cliente Req 91200------------------
    is_clie_pedido   varchar2(100):=NULL;
    is_reserva_mod   number:=0;
    is_estado_pedido varchar2(1):=NULL;
  BEGIN
    --<i84905>Color interno
    ...
    -------------------------------------
    ---Liberar Reserva
    /*Inicio Req 91200 ebarboza 28 03 2021*/
    BEGIN
      BEGIN
      SELECT p.cod_estado_pedido_veh INTO is_estado_pedido FROM vve_pedido_veh p WHERE p.num_pedido_veh = p_num_pedido_veh;
       EXCEPTION
        WHEN OTHERS THEN
          is_estado_pedido:=NULL;
       END;

       IF is_estado_pedido = 'R' THEN
         SELECT rp.cod_clie INTO is_clie_pedido FROM vve_pedido_veh_reserva rp WHERE rp.num_pedido_veh = p_num_pedido_veh
        AND rp.cod_estado_reserva_pedido = '001' AND rp.cod_clie in (SELECT prf.cod_clie FROM vve_proforma_veh prf WHERE prf.num_prof_veh = p_num_prof_veh
        AND not prf.cod_estado_prof = 'I');
    ...
    EXCEPTION
      WHEN OTHERS THEN
        p_ret_mens := 'No es posible liberar la reserva, porque le pertenece a otro cliente';
        is_reserva_mod:=0;
        p_ret_esta:=0;
        RAISE ve_error;
    END;
     /*Fin Req 91200 ebarboza 28 03 2021*/
    -------------------------------------
    ...
    /*Inicio Req 91200 ebarboza 28 03 2021*/ 
     ...
     WHERE cod_cia = p_cod_cia
       ...
       AND num_pedido_veh = p_num_pedido_veh
       AND cod_estado_pedido_veh = 'R'
       AND 1 = is_reserva_mod;
         EXCEPTION
           WHEN OTHERS THEN
           is_estado_pedido :=NULL;
           p_ret_mens := 'No se pude preasignar el pedido a la ficha';
           is_reserva_mod:=0;
           p_ret_esta:=0;   
       END;
     ...
     ----------Asignar pedido a la ficha de venta------------------
     BEGIN
      SELECT p.cod_estado_pedido_veh INTO is_estado_pedido FROM vve_pedido_veh p WHERE p.num_pedido_veh = p_num_pedido_veh;
      EXCEPTION
        WHEN OTHERS THEN
     ...
     IF (p_nur_ficha_vta_pedido IS NULL OR p_nur_ficha_vta_pedido = 0) AND is_estado_pedido = 'P' THEN
      BEGIN
        SELECT nvl(MAX(nur_ficha_vta_pedido), 0) + 1
          INTO v_nur_ficha_vta_pedido
          FROM vve_ficha_vta_pedido_veh
...
--<F-88431-2>
  ...
  (
    ...
    is_reserva_mod   number:=0;
    is_estado_pedido varchar2(1):=NULL;
    -----------------Valida que no se halla excluido el modelo en la FCC de Sid-Tree
    fcc_modelo_excluido_sid_tree NUMBER:=0;
  BEGIN
...
pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_SP',
    ...
    --/*Inicio Req. 92036 ebarboza 23 05 2021*/
    SELECT 
     (PKG_GEN_SELECT.FUNC_LVAL_AREA_FAMILIA_MARCA(ax.cod_cia,'EXCLUFCL',ax.cod_baumuster,ax.cod_familia_veh,ax.cod_area_vta)) INTO fcc_modelo_excluido_sid_tree
     FROM vve_pedido_veh ax
     inner join vve_ficha_vta_pedido_veh bx
     on bx.num_pedido_veh = ax.num_pedido_veh
     AND bx.cod_cia = ax.cod_cia
     AND ax.cod_prov = ax.cod_prov
     AND bx.ind_inactivo = 'N'
     WHERE ax.num_pedido_veh = p_num_pedido_veh
     AND bx.num_ficha_vta_veh = p_num_ficha_vta_veh;     
   --/*Fin Req. 92036 ebarboza 23 05 2021*/
    EXCEPTION
      WHEN OTHERS THEN
        l_is_auto := 0;
    ...
    IF (l_is_auto = 0) OR (fcc_modelo_excluido_sid_tree = 1) THEN
      UPDATE vve_ficha_vta_veh_aut a
    ...
    FROM (SELECT y.*
      FROM (SELECT x.*
            FROM (SELECT a.cod_cia,
                       ...
                       a.cod_area_vta,
                     --/*Inicio Req. 92036 ebarboza 23 05 2021*/
                       PKG_GEN_SELECT.FUNC_LVAL_AREA_FAMILIA_MARCA(a.cod_cia,''EXCLUFCL'',a.cod_baumuster,a.cod_familia_veh,a.cod_area_vta) fcc_modelo_excluido_sid_tree,
                     --/*Fin Req. 92036 ebarboza 23 05 2021*/
                              ...
                              Where ep.cod_estado_pedido      = a.cod_estado_pedido_veh
                                ...
                                And ( nvl(epa.Asigna_x_cliente ,''N'') = ''N'' or Exists (Select 1 from VVE_PEDIDO_VEH_RESERVA re
                                                                                    Where re.cod_cia  = a.cod_cia
                                                                                      ...
                                                                                      /*<I-92416_L_Mejorar_crear_fdv_con_reserva_para_autos_preasignar_pedidos>*/
                                                                                      AND re.fec_fin_reserva_pedido >= trunc(SYSDATE)
                                                                                      AND re.cod_estado_reserva_pedido = ''001''
                                                                                      /*<F-92416_L_Mejorar_crear_fdv_con_reserva_para_autos_preasignar_pedidos>*/
                                                                                      And re.cod_clie = ''' ||
                    l_cod_clie || '''
                                                                                 )
                                  )
