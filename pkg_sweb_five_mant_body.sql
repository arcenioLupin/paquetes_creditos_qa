create or replace PACKAGE BODY       pkg_sweb_five_mant AS
  /******************************************************************************
  ...
  *********************************************************************************/
  ...
  END sp_fun_str_prof_ficha_vta;
   ...
   /********************************************************************************
      Nombre:     SP_FUN_AGRUPAR_BONOS_FICHA
      Proposito:  PERMITIRA AGRUPAR TODOS LOS BONOS DE UNA FICHA DE VENTA EN UNA SOLA LINEA.
      Referencias: Req. 92349 MEJORAS EN BONOS DE POST-VENTA 21 06 2021


      REVISIONES:
      Version    Fecha       Autor            Descripcion
      ---------  ----------  ---------------  ------------------------------------
      1.0         03/06/2021  EBARBOZA       Creación del procedure.
  *********************************************************************************/
   FUNCTION SP_FUN_AGRUPAR_BONOS_FICHA(p_ficha_vta_veh VARCHAR2,p_num_prof_veh VARCHAR2,p_campo_return VARCHAR2)
    RETURN VARCHAR2 IS
    wn_resultado VARCHAR2(3000);
    cursor_ficha_vta        SYS_REFCURSOR;
    ws_cod_vale_five_bono   VARCHAR2(1000) := '';
    ws_cod_five_bono        VARCHAR2(1000) := '';
    ws_return_ficha_vta     VARCHAR2(1000) := '';
    t_data_compare VARCHAR2(1000) := '';
  BEGIN
    OPEN cursor_ficha_vta FOR SELECT DISTINCT fbv.cod_vale_five_bono, fbv.cod_five_bono
                              FROM
                                  vve_ficha_vta_veh fv
                                  LEFT OUTER JOIN vve_ficha_vta_proforma_veh fvp ON fvp.num_ficha_vta_veh = fv.num_ficha_vta_veh
                                  LEFT OUTER JOIN vve_proforma_veh pv ON pv.num_prof_veh = fvp.num_prof_veh
                                  LEFT OUTER JOIN vve_five_bono vfb ON vfb.num_ficha_vta_veh = fv.num_ficha_vta_veh
                                                                       AND vfb.num_prof_veh = pv.num_prof_veh
                                  LEFT OUTER JOIN vve_five_bono_vale fbv ON fbv.num_ficha_vta_veh = fv.num_ficha_vta_veh
                                                                            AND fbv.num_prof_veh = pv.num_prof_veh
                                  LEFT OUTER JOIN vve_prof_equipo_local_veh pel ON pel.num_prof_veh = pv.num_prof_veh
                                  LEFT OUTER JOIN vve_equipo_local_veh elv ON elv.cod_equipo_local_veh = pel.cod_equipo_local_veh
                              WHERE
                                  fvp.ind_inactivo = 'N'
                                  AND pel.ind_inactivo = 'N'
                                  AND NOT fv.cod_estado_ficha_vta_veh IN ('I')
                                  AND NOT pv.cod_estado_prof IN ('I')
                                  AND fv.num_ficha_vta_veh = p_ficha_vta_veh
                                  AND pv.num_prof_veh = p_num_prof_veh
                                  AND elv.cod_tipo_equipo_local_veh IN ('94');

    LOOP
        FETCH cursor_ficha_vta INTO  ws_cod_vale_five_bono,ws_cod_five_bono;
        EXIT WHEN cursor_ficha_vta%notfound;
        SELECT DISTINCT
            ( CASE p_campo_return
                WHEN 'MONTO_TOTAL_BONO'     THEN TO_CHAR(round(a.monto_total_bono,2) )
                WHEN 'COD_USUARIO_CREA'     THEN TO_CHAR(a.cod_usuario_crea)
                WHEN 'NOMBRES'              THEN TO_CHAR( (u.txt_apellidos
                                                || ', '
                                                || u.txt_nombres) )
                WHEN 'FEC_CREA_REG_FICHA'   THEN TO_CHAR(a.fec_crea_reg,'DD/MM/RRRR')
                WHEN 'ESTADO_BONO_FICHA'    THEN DECODE(b.cod_esta_vale,'A','Aprobar','I','Inactivar','R','Rechazar','P','Pendiente')
            END )
        INTO ws_return_ficha_vta
        FROM
            vve_five_bono a
            INNER JOIN vve_five_bono_vale b ON b.cod_five_bono = a.cod_five_bono
                                               AND b.num_ficha_vta_veh = a.num_ficha_vta_veh
                                               AND b.num_prof_veh = a.num_prof_veh
            INNER JOIN vve_ficha_vta_proforma_veh fpv ON ( fpv.num_ficha_vta_veh = b.num_ficha_vta_veh
                                                           OR fpv.num_ficha_vta_veh = a.num_ficha_vta_veh )
            INNER JOIN vve_prof_equipo_local_veh pel ON pel.num_prof_veh = fpv.num_prof_veh
            INNER JOIN vve_equipo_local_veh elv ON elv.cod_equipo_local_veh = pel.cod_equipo_local_veh
            INNER JOIN sistemas.sis_mae_usuario u ON u.cod_id_usuario = a.cod_usuario_crea
        WHERE
            a.cod_five_bono = b.cod_five_bono
            AND elv.cod_tipo_equipo_local_veh IN ('94')
            AND a.ind_inactivo = 'N'
            AND b.ind_inactivo = 'N'
            AND a.num_ficha_vta_veh IN (
                p_ficha_vta_veh
            )
            AND a.cod_five_bono = ws_cod_five_bono
            AND b.cod_vale_five_bono = ws_cod_vale_five_bono
            AND b.cod_five_bono = ws_cod_five_bono;

        IF  t_data_compare != ws_return_ficha_vta THEN
          IF length(wn_resultado) > 0 THEN
              wn_resultado := wn_resultado || ' / ';
          END IF;
          wn_resultado := wn_resultado || ws_return_ficha_vta;
          ELSE
          wn_resultado :=  ws_return_ficha_vta;  
        END IF;
        t_data_compare := Ltrim(Rtrim(Substr(ws_return_ficha_vta,1,LENGTH(ws_return_ficha_vta))));

        dbms_output.put_line(wn_resultado);
    END LOOP;
    CLOSE cursor_ficha_vta;
          return wn_resultado;
EXCEPTION
    WHEN no_data_found THEN
             return wn_resultado;
        dbms_output.put_line('AUDI_ERROR'
                                           || ' SP_FUN_AGRUPAR_BONOS_PROFORMA '
                                           || user
                                           || ' ERROR NO CONTROLADO '
                                           || sqlerrm
                                           || ' p_ficha_vta_veh:>> '||p_ficha_vta_veh||
                                          ' ws_cod_vale_five_bono:>> '||ws_cod_vale_five_bono||
                                          ' ws_cod_five_bono:>> '||ws_cod_five_bono||
                                          ' ws_return_ficha_vta:>> '||ws_return_ficha_vta||
                                          ' p_campo_return:>> '||p_campo_return);
    WHEN OTHERS THEN
           return wn_resultado;
        pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_ERROR',
                                          'SP_FUN_AGRUPAR_BONOS_FICHA',
                                          USER,
                                          'ERROR NO CONTROLADO',
                                          ' p_ficha_vta_veh:>> '||p_ficha_vta_veh||
                                          ' ws_cod_vale_five_bono:>> '||ws_cod_vale_five_bono||
  ...
  END SP_FUN_AGRUPAR_BONOS_FICHA;

  /********************************************************************************
      Nombre:     SP_FUN_AGRUPAR_BONOS_PROFORMA
      Proposito:  PERMITIRA AGRUPAR TODOS LOS BONOS DE UNA FICHA DE VENTA EN UNA SOLA LINEA.
      Referencias: Req. 92349 MEJORAS EN BONOS DE POST-VENTA 21 06 2021
      ...
      1.0         03/06/2021  EBARBOZA       Creación del procedure.
  *********************************************************************************/
  FUNCTION sp_fun_agrupar_bonos_proforma (p_ficha_vta_veh VARCHAR2,p_campo_return VARCHAR2) 
    RETURN VARCHAR2 IS
        wn_resultado              VARCHAR2(3000);
        cursor_proforma           SYS_REFCURSOR;
        ws_num_prof_veh           VARCHAR2(1000) := '';
        ws_cod_equipo_local_veh   VARCHAR2(1000) := '';
        ws_return_proforma        VARCHAR2(1000) := '';
        t_data_compare VARCHAR2(1000) := '';
    BEGIN
        OPEN cursor_proforma FOR SELECT
                                     fvp.num_prof_veh,
                                     elv.cod_equipo_local_veh
                                 FROM
                                     vve_ficha_vta_proforma_veh fvp
                                     LEFT OUTER JOIN vve_ficha_vta_veh fv ON fv.num_ficha_vta_veh = fvp.num_ficha_vta_veh
                                     LEFT OUTER JOIN vve_prof_equipo_local_veh pel
                                     LEFT OUTER JOIN vve_proforma_veh p ON p.num_prof_veh = pel.num_prof_veh ON pel.num_prof_veh =
                                     fvp.num_prof_veh
                                     LEFT OUTER JOIN vve_equipo_local_veh elv ON elv.cod_equipo_local_veh = pel.cod_equipo_local_veh
                                 WHERE
                                     fvp.ind_inactivo = 'N'
                                     AND pel.ind_inactivo = 'N'
                                     AND NOT p.cod_estado_prof IN ('I')
                                     AND NOT fv.cod_estado_ficha_vta_veh IN ('I')
                                     AND fv.num_ficha_vta_veh = p_ficha_vta_veh
                                     AND elv.cod_tipo_equipo_local_veh IN ('94');
        ...
        LOOP
            BEGIN
                FETCH cursor_proforma INTO
                    ws_num_prof_veh,
                    ws_cod_equipo_local_veh;
                EXIT WHEN cursor_proforma%notfound;
                SELECT DISTINCT
                    ( CASE p_campo_return
                        WHEN 'DES_TIPO_EQUIPO_LOCAL_VEH'   THEN tel.des_tipo_equipo_local_veh
                        WHEN 'COD_EQUIPO_LOCAL_VEH'        THEN a.cod_equipo_local_veh
                        WHEN 'DES_EQUIPO_LOCAL_VEH'        THEN f.des_equipo_local_veh
                        WHEN 'CAN_EQUIPO_LOCAL_VEH'        THEN TO_CHAR(a.can_equipo_local_veh)
                        WHEN 'VAL_EQUIPO_LOCAL_VEH'        THEN TO_CHAR(round(a.val_equipo_local_veh,2) )
                        WHEN 'VAL_TOT_EQUIPO_LOCAL_VEH'    THEN TO_CHAR(round(a.val_tot_equipo_local_veh) )
                        WHEN 'NOM_MARCA'                   THEN g.nom_marca
                        WHEN 'COD_BAUMUSTER'               THEN c.cod_baumuster
                        WHEN 'DES_BAUMUSTER'               THEN h.des_baumuster
                        WHEN 'COD_CONFIG_VEH'              THEN c.cod_config_veh
                        WHEN 'DES_CONFIG_VEH'              THEN i.des_config_veh
                        WHEN 'COD_CLASE_VEH'               THEN h.cod_clase_veh
                        WHEN 'IND_NUEVO_USADO'             THEN c.ind_nuevo_usado
                        WHEN 'NUM_PROF_VEH'                THEN pv.num_prof_veh
                    END )
                INTO ws_return_proforma
                FROM
                    vve_ficha_vta_proforma_veh fvp
                    INNER JOIN vve_proforma_veh pv ON pv.num_prof_veh = fvp.num_prof_veh
                                                      AND NOT pv.cod_estado_prof IN (
                        'I'
                    )
                    INNER JOIN vve_proforma_veh_det pvd ON pvd.num_prof_veh = pv.num_prof_veh
                                                           AND pvd.num_reg_det_prof IS NOT NULL
                    INNER JOIN vve_prof_equipo_local_veh a ON a.num_prof_veh = pv.num_prof_veh
                                                              AND a.num_reg_det_prof = pvd.num_reg_det_prof
                                                              AND a.ind_inactivo = 'N'
                    INNER JOIN vve_ficha_vta_pedido_veh b ON b.num_ficha_vta_veh = fvp.num_ficha_vta_veh
                                                             AND b.ind_inactivo = 'N'
                    INNER JOIN vve_pedido_veh c ON c.num_pedido_veh = b.num_pedido_veh
                                                   AND c.cod_prov = b.cod_prov
                                                   AND c.cod_cia = b.cod_cia
                                                   AND NOT c.cod_estado_pedido_veh IN ('I')
                    INNER JOIN vve_equipo_local_veh f ON f.cod_equipo_local_veh = a.cod_equipo_local_veh
                                                         AND f.cod_prov = a.cod_prov
                                                         AND f.ind_inactivo = 'N'
                    INNER JOIN vve_tipo_equipo_local_veh tel ON tel.cod_tipo_equipo_local_veh = f.cod_tipo_equipo_local_veh
                                                                AND tel.ind_inactivo = 'N'
                    INNER JOIN vve_config_veh i ON i.cod_marca = c.cod_marca
                                                   AND i.cod_baumuster = c.cod_baumuster
                                                   AND i.cod_familia_veh = c.cod_familia_veh
                                                   AND i.cod_config_veh = c.cod_config_veh
                                                   AND i.ind_inactivo = 'N'
                    INNER JOIN gen_marca g ON g.cod_marca = c.cod_marca
                    INNER JOIN vve_baumuster h ON h.cod_baumuster = c.cod_baumuster
                    INNER JOIN gen_area_vta j ON j.cod_area_vta = pv.cod_area_vta
                    INNER JOIN gen_filiales m ON m.cod_filial = pv.cod_filial
                    INNER JOIN arfamc l ON l.no_cia = pv.cod_cia
                WHERE
                    pv.num_prof_veh = ws_num_prof_veh
                    AND f.cod_equipo_local_veh = ws_cod_equipo_local_veh
                    AND f.cod_tipo_equipo_local_veh IN ('94');

                IF  t_data_compare != ws_return_proforma THEN
          IF length(wn_resultado) > 0 THEN
              wn_resultado := wn_resultado || ' / ';
          END IF;
          wn_resultado := wn_resultado || ws_return_proforma;
          ELSE
          wn_resultado :=  ws_return_proforma;  
        END IF;
        t_data_compare := Ltrim(Rtrim(Substr(ws_return_proforma,1,LENGTH(ws_return_proforma))));
                dbms_output.put_line(wn_resultado);
            EXCEPTION
                WHEN no_data_found THEN
                    dbms_output.put_line('AUDI_ERROR'
       ...
       /*  pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_ERROR',
                                          ...
                                          'ERROR NO CONTROLADO',
                                          ' p_ficha_vta_veh:>> '||p_ficha_vta_veh||
                                          ' ws_num_prof_veh:>> '||ws_num_prof_veh||
                                          ' ws_return_proforma:>> '||ws_return_proforma||
                                          ' WS_COD_EQUIPO_LOCAL_VEH:>> '||WS_COD_EQUIPO_LOCAL_VEH||
                                          ' p_campo_return:>> '||p_campo_return,
                                           SQLERRM);*/
                WHEN OTHERS THEN
                    pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_ERROR',
        ...
        CLOSE cursor_proforma;
        RETURN wn_resultado;
    END sp_fun_agrupar_bonos_proforma;

    /********************************************************************************
      Nombre: SP_FICHA_VENTA_REPORTE_BONOS
      Proposito:  Permite obtener la informacion de los bonos en una ficha de venta
      que se han dados en las proforma.
      Referencias: Req. 92349 MEJORAS EN BONOS DE POST-VENTA 21 06 2021

      REVISIONES:
      Version    Fecha       Autor            Descripcion
      ---------  ----------  ---------------  ------------------------------------
      1.0         17/06/2021  EBARBOZA        Creación del procedure.
  *********************************************************************************/
  PROCEDURE SP_FICHA_VENTA_REPORTE_BONOS
  (
    p_cod_cia                  IN VARCHAR2,
    p_cod_area_vta             IN VARCHAR2,
    p_cod_filial               IN VARCHAR2,
    ...
    v_final          VARCHAR2(10000);
    --v_ind_vendedores NUMBER;
    l_ind_reporte_ficha_vta_bonos VARCHAR2(10000);
    v_pre_final          VARCHAR2(32000);
  BEGIN

  SELECT decode(COUNT(*), 0, 'N', 'S')
        INTO l_ind_reporte_ficha_vta_bonos
        FROM sis_mae_perfil_procesos a
       INNER JOIN sis_mae_perfil_usuario b
          ON a.cod_id_perfil = b.cod_id_perfil
       INNER JOIN SIS_MAE_USUARIO c
          ON c.cod_id_usuario = b.cod_id_usuario
       WHERE a.cod_id_procesos = 161
         AND upper(c.txt_usuario) = upper(p_cod_usua_sid)
         AND a.ind_realiza = 'S'
         AND nvl(a.ind_inactivo, 'N') = 'N'
         AND nvl(b.ind_inactivo, 'N') = 'N';
   IF l_ind_reporte_ficha_vta_bonos = 'N' THEN
    p_ret_cantidad:= 0;
    p_ret_esta:= 0;
    p_ret_mens:='Usted no cuenta con permiso';-- : Perfil ROL_REPORTE_FICHA_VTA_BONOS
    RAISE ve_error;
   END IF;   

  pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_SQL',
                                        'SP_REPORTE_FICHA_VENTA_BONOS',
                                        p_cod_usua_sid,
    ...
    v_query      := 'SELECT DISTINCT 
     ...
     FVF.importe_factura,
     FVF.nom_perso,
     FVF.fecha_facturacion,
     FVF.fecha_cancelacion,
     FVF.doc_devol,
     FVF.no_devol,
     FVF.des_equipo_local_veh,
     FVF.cod_equipo_local_veh,
     FVF.can_equipo_local_veh,
     FVF.val_equipo_local_veh,
     FVF.fecha_bono,
     FVF.anio,
     NVL(FVF.estado_bono_ficha,''Sin Registrar'') as estado_bono_ficha,
     FVF.num_ficha_vta_veh,
     FVF.FEC_FICHA_VTA_VEH,
     FVF.COD_ESTADO_FICHA_VTA_VEH,
     FVF.des_estado_ficha_vta,
     FVF.num_prof_veh,
     FVF.des_estado_prof,
     FVF.nom_marca,
     FVF.des_baumuster,
     FVF.fec_entrega_clie,
     FVF.cod_tipo_equipo_local_veh
             ...
             rf.tipo_doc,
             rf.no_factu,
             rf.val_pre_docu    AS importe_factura,
             o.nom_perso,
             rf.fec_crea_reg    AS fecha_facturacion,
             pvp.fec_crea_reg   AS fecha_cancelacion,
             rf.doc_devol,
             rf.no_devol,
             elv.des_equipo_local_veh,
             elv.cod_tipo_equipo_local_veh,
             pel.can_equipo_local_veh,
             pel.cod_equipo_local_veh,
             pel.val_equipo_local_veh,
             pel.fec_crea_reg   AS fecha_bono,
             TO_CHAR(pel.fec_crea_reg,''RRRR'') AS anio,
             PKG_SWEB_FIVE_MANT.SP_FUN_AGRUPAR_BONOS_FICHA(fv.NUM_FICHA_VTA_VEH,pv.num_prof_veh,''ESTADO_BONO_FICHA'') AS estado_bono_ficha,
             fv.num_ficha_vta_veh,
             fv.COD_ESTADO_FICHA_VTA_VEH,
             fv.fec_crea_reg    AS FEC_FICHA_VTA_VEH,
             ext.des_estado_ficha_vta,
             pv.num_prof_veh,
             expx.des_estado_prof,
             gm.nom_marca,
             vbx.des_baumuster,
             p.fec_entrega_clie
         FROM
             vve_ficha_vta_veh fvf
             INNER JOIN arccve v ON fvf.vendedor = v.vendedor
             INNER JOIN gen_sucursales s ON fvf.cod_sucursal = s.cod_sucursal
             INNER JOIN arfamc c ON fvf.cod_cia = c.no_cia
             INNER JOIN gen_filiales l ON fvf.cod_filial = l.cod_filial
             INNER JOIN gen_area_vta g ON fvf.cod_area_vta = g.cod_area_vta
             INNER JOIN vve_tipo_ficha_vta_veh t ON fvf.cod_tipo_ficha_vta_veh = t.cod_tipo_ficha_vta_veh
             INNER JOIN vve_estado_ficha_vta d ON fvf.cod_estado_ficha_vta_veh = d.cod_estado_ficha_vta
             INNER JOIN gen_persona o ON fvf.cod_clie = o.cod_perso
             INNER JOIN vve_tipo_pago tp ON fvf.cod_tipo_pago = tp.cod_tipo_pago
             LEFT OUTER JOIN vve_ficha_vta_proforma_veh fp ON fvf.num_ficha_vta_veh = fp.num_ficha_vta_veh
             LEFT OUTER JOIN vve_ficha_vta_veh fv ON fv.num_ficha_vta_veh = fp.num_ficha_vta_veh
             LEFT OUTER JOIN vve_estado_ficha_vta ext ON ext.cod_estado_ficha_vta = fv.cod_estado_ficha_vta_veh
             LEFT OUTER JOIN v_ficha_vta_veh_aut fva ON fvf.num_ficha_vta_veh = fva.num_ficha_vta_veh
             LEFT OUTER JOIN vve_proforma_veh_det pd ON fp.num_prof_veh = pd.num_prof_veh
             LEFT OUTER JOIN vve_proforma_veh pv ON pv.num_prof_veh = pd.num_prof_veh
             LEFT OUTER JOIN vve_estado_proforma expx ON expx.cod_estado_prof = pv.cod_estado_prof
             LEFT OUTER JOIN vve_ficha_vta_pedido_veh fpv ON fpv.num_ficha_vta_veh = fvf.num_ficha_vta_veh
                                                             AND fpv.ind_inactivo = ''N''
             LEFT OUTER JOIN vve_pedido_veh p ON fpv.num_pedido_veh = p.num_pedido_veh
             LEFT OUTER JOIN vve_adquisicion a ON p.cod_adquisicion_pedido_veh = a.cod_adquisicion
             LEFT OUTER JOIN vve_prof_equipo_local_veh pel ON pel.num_prof_veh = pd.num_prof_veh
             LEFT OUTER JOIN vve_equipo_local_veh elv ON elv.cod_equipo_local_veh = pel.cod_equipo_local_veh AND elv.cod_tipo_equipo_local_veh in (''94'')
             LEFT OUTER JOIN gen_marca gm ON gm.cod_marca = p.cod_marca
             LEFT OUTER JOIN vve_baumuster vbx ON vbx.cod_baumuster = p.cod_baumuster
             LEFT OUTER JOIN arfafe rf ON rf.no_orden_desc = p.num_pedido_veh
             LEFT OUTER JOIN vve_five_bono fvb ON fvb.num_ficha_vta_veh = fv.num_ficha_vta_veh
                                                  AND fvb.num_prof_veh = pv.num_prof_veh
             LEFT OUTER JOIN vve_five_bono_vale fbv ON fbv.num_ficha_vta_veh = fv.num_ficha_vta_veh
                                                       AND fbv.num_prof_veh = pv.num_prof_veh
                                                       AND fbv.cod_five_bono = fvb.cod_five_bono
             LEFT OUTER JOIN vve_pedido_veh_pagos pvp ON pvp.num_pedido_veh = p.num_pedido_veh
                                                         AND pvp.num_docu = rf.no_factu
         GROUP BY
             c.nombre,
             p.num_pedido_veh,
             p.vhcle_sap,
             ...
             elv.cod_tipo_equipo_local_veh
                     ...
                     left JOIN vve_proforma_veh_det c
                              ON a.num_prof_veh = c.num_prof_veh
                     WHERE FVF.cod_tipo_equipo_local_veh in (''94'') AND ';
   ...
   /* IF v_ind_vendedores = 0 THEN
    ...
    v_order := ' ORDER BY FVF.FEC_FICHA_VTA_VEH DESC ';
    pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_SQL',
                                        'SP_REPORTE_FICHA_VENTA_BONOS',
                                        p_cod_usua_sid,
                                        NULL,
    ...
    EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM (' || v_query || ')'
      ...
      IF p_ret_cantidad > 0 THEN
        p_ret_esta := 1;
        p_ret_mens := 'La consulta se realizó de manera exitosa';
      ELSE
        p_ret_cantidad:= 0;
        p_ret_esta:= 0;
        p_ret_mens:='No se encontraron registro para su solicitud';
        RAISE ve_error;
      END IF;
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      pkg_sweb_mae_gene.sp_regi_rlog_erro('AUDI_ERROR',
                                          ...
                                          p_ret_mens,
                                          p_num_ficha_vta_veh);
  END SP_FICHA_VENTA_REPORTE_BONOS;
